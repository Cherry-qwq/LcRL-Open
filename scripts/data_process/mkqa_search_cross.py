...
You can search at least 2 times: first search in {lang_name} (or a high resource language) knowledge base, and second search in other high resource languages like {high_resource_lang} to provide more comprehensive information. If needed, continue searching in English \
If you find no further external knowledge needed, you can directly provide the answer inside <answer> and </answer>, without detailed illustrations. \
Note: The answer should be in {answer_lang_name}. For example, <answer> {example_answer} </answer>. 

Question: {question}"""
    elif template_type == 'cross-new':
        prefix = f"""Answer the given question. \
You must conduct reasoning inside <think> and </think> first every time you get new information. \
After reasoning, if you find you lack some knowledge, you can call a search engine by <search> and </search>, and it will return the top searched results between <information> and </information>. \
You can search at least 2 times: first search in {lang_name} knowledge base, and second search in other languages like {high_resource_lang} to **compare with the first-round results and resolve any factual conflicts or biases within <think> and </think>**. If needed, continue searching in English \
If you find no further external knowledge needed, you can directly provide the answer inside <answer> and </answer>, without detailed illustrations. \
Note: The answer should be in {answer_lang_name}. For example, <answer> {example_answer} </answer>. 

Question: {question}"""
    elif template_type == 'cross-new-2':
        prefix = f"""Answer the given question. \
You must conduct reasoning inside <think> and </think> first every time you get new information. \
After reasoning, if you find you lack some knowledge, you can call a search engine by <search> and </search>, and it will return the top searched results between <information> and </information>. \
You can search at least 2 times: first search in {lang_name} knowledge base, and second search in other languages like {high_resource_lang} to **reconstruct global evidence into brief {answer_lang_name} fact-statements inside <think> and </think>** to resolve knowledge conflicts. If needed, continue searching in English \
If you find no further external knowledge needed, you can directly provide the answer inside <answer> and </answer>, without detailed illustrations. \
Note: The answer should be in {answer_lang_name}. For example, <answer> {example_answer} </answer>. 
...
def create_dataset(df, language, split, template_type='base'):
    ...
    for idx, row in enumerate(df.itertuples(), start=0):
        ...
        example = {
            ...
            "example_id": row.example_id  # 确保保留 example_id
        }
...
def process_language(file_path, language, template_type, local_dir, test_ids, hdfs_dir=None):
    """
    处理单个语言的数据：
    1. 读取 jsonl 为 DataFrame
    2. 过滤掉 ground truth 含 null 的样本
    3. [修改]: 根据 test_ids 将数据分为 Train (不包含 test_ids) 和 Test (包含 test_ids)
    """
    print(f"\n{'='*60}")
    ...
    # [修改] 使用 example_id 进行集合操作来切分
    # 确保列名存在
    if 'example_id' not in df.columns:
        raise ValueError("Column 'example_id' not found in mkqa.json!")

    # 创建掩码：如果在 test_ids 中，则为 True (Test Set)，否则为 False (Train Set)
    mask_test = df['example_id'].isin(test_ids)
    
    test_df = df[mask_test].reset_index(drop=True)
    train_df = df[~mask_test].reset_index(drop=True)
    
    print(f"[Split] Based on provided test IDs:")
    print(f"       Test examples found in current data: {len(test_df)}")
    print(f"       Train examples (remaining): {len(train_df)}")
    ...
    train_file = os.path.join(local_dir, f'train_{language}_compare.parquet')
    test_file = os.path.join(local_dir, f'test_{language}_compare.parquet')
    ...
    if train_data:
        ...
        print(f"[WARNING] Train data is empty for {language}!")

    # 保存测试集（如果该语言下有匹配的 test_ids）
    if test_data:
        pd.DataFrame(test_data).to_parquet(test_file)
        print(f"✓ Saved Test Set: {test_file}")
    else:
        print(f"[INFO] No matching test data found for {language} (or all filtered out).")
    ...
    if hdfs_dir is not None:
        ...
        if os.path.exists(train_file):
            copy(src=train_file, dst=os.path.join(hdfs_dir, f'train_{language}_compare.parquet'))
        if os.path.exists(test_file):
            copy(src=test_file, dst=os.path.join(hdfs_dir, f'test_{language}_compare.parquet'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--template_type', type=str, default='base', 
                       choices=['base', 'multilang_instruction', 'base-hr', 'cross-new', 'cross-new-2'],
    ...
    # [修改] 新增参数：测试集参考文件路径
    parser.add_argument('--test_reference_file', type=str,
                       default='/local_data/ruiqi/Search-R1/data/mkqa/parallel/mkqa_test_ar_1277.jsonl',
                       help='Path to the existing test set jsonl file used to exclude IDs from training set')

    args = parser.parse_args()
    
    #定义支持的所有语言
    # all_languages = ['en', 'fr', 'es', 'de', 'zh_cn', 'ja', 'ar', 'ru', 'pt', 'it', 'ko', 'fi', 'th']
    all_languages = ['en', 'fr', 'ar', 'th']
    ...
    # [修改] 1. 加载测试集 ID 列表
    print(f"\n{'='*60}")
    print(f"Loading Test IDs from: {args.test_reference_file}")
    test_ids_set = set()
    try:
        with open(args.test_reference_file, 'r', encoding='utf-8') as f:
            for line in f:
                if not line.strip(): continue
                item = json.loads(line)
                if 'example_id' in item:
                    test_ids_set.add(item['example_id'])
        print(f"✓ Successfully loaded {len(test_ids_set)} unique test IDs.")
    except Exception as e:
        print(f"[ERROR] Failed to load test reference file: {e}")
        exit(1)
    print(f"{'='*60}")
    ...
    for language in languages_to_process:
        try:
            process_language(
                ...
                test_ids=test_ids_set, # [修改] 传入 ID 集合
