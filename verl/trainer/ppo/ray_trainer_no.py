...
def _norm_lang(s: str) -> str:
    s = (s or 'en').lower().replace('_', '-').strip()
    if s in ('zh-cn', 'zh_cn', 'zh-hans', 'zhs'):
        return 'zh'
    return s.split('-')[0]
...
def _norm_lang_for_log(s: str) -> str:
    if not s:
        return "unk"
    s = s.lower().strip().replace("_", "-")
    if s in ("zh-cn", "zh_cn", "zh-hans", "zhs"):
        return "zh"
    return s.split("-")[0]  # pt-br -> pt
...
class RayPPOTrainer(object):
    ...
    def _build_grpo_search_languages(self, batch: DataProto) -> list[str]:
        """
        为GRPO构造question_languages：
        - 第1个采样(seen=0): 使用原语言
        - 第2个采样(seen=1): 使用英语 'en'
        - 第3个采样(seen=2): 使用高资源语言A
        """
        ...
        for lang_raw, idx in zip(raw_langs, indices):
            ...
            if n_agent >= 3:
                if seen == 0:
                    search_lang = base_lang  # 第1次：原语言
                elif seen == 1:
                    search_lang = 'en'  # 第2次：英语
                elif seen == 2:
                    base_norm = _norm_lang(base_lang)
                    search_lang = _LANGUAGE_TO_HIGH_RESOURCE.get(base_norm, 'en')  # 第3次：高资源
                else:
    ...
    def _prepare_multilingual_grpo_batch(self, batch: DataProto) -> DataProto:
        """
        为GRPO准备3个不同语言版本的batch
        - 版本1: 使用 content (原语言)
        - 版本2: 使用 content_en (英语)
        - 版本3: 使用 content_source (高资源语言)
        
        Args:
            batch: 原始batch (batch_size=B)
        
        Returns:
            扩展后的batch (batch_size=B*3, interleaved)
        """
        batch_size = len(batch.batch['input_ids'])
        n_agent = self.config.actor_rollout_ref.rollout.n_agent
        
        if n_agent != 3:
            print(f"[WARNING] Multilingual GRPO expects n_agent=3, got {n_agent}. Falling back to standard repeat.")
            return batch.repeat(repeat_times=n_agent, interleave=True)
        
        # 检查必要字段
        required_fields = ['content', 'content_en', 'content_source']
        for field in required_fields:
            if field not in batch.non_tensor_batch:
                print(f"[WARNING] Missing {field} in batch, falling back to standard repeat.")
                return batch.repeat(repeat_times=n_agent, interleave=True)
        
        # 获取3个版本的content
        contents_orig = batch.non_tensor_batch['content']
        contents_en = batch.non_tensor_batch['content_en']
        contents_source = batch.non_tensor_batch['content_source']
        
        # 获取原始的 raw_prompt (如果存在)
        raw_prompts = batch.non_tensor_batch.get('raw_prompt', None)
        
        # 准备存储3个版本的tokenized数据
        all_input_ids = []
        all_attention_mask = []
        all_position_ids = []
        
        # 对每个样本生成3个版本（interleave方式）
        for i in range(batch_size):
            # 获取当前样本的3个版本文本
            texts = [
                contents_orig[i],    # 版本1: 原语言
                contents_en[i],      # 版本2: 英语
                contents_source[i]   # 版本3: 高资源语言
            ]
            ...
            for text in texts:
                ...
                if raw_prompts is not None and i < len(raw_prompts):
                    try:
                        # 尝试复制原始 chat 结构
                        original_chat = raw_prompts[i]
                        if isinstance(original_chat, list):
                            chat = [msg.copy() if isinstance(msg, dict) else msg for msg in original_chat]
                            # 修改第一条消息的 content
                            if len(chat) > 0 and isinstance(chat[0], dict):
                                chat[0]['content'] = str(text)
                            else:
                                chat = [{"role": "user", "content": str(text)}]
                        else:
                            chat = [{"role": "user", "content": str(text)}]
        ...
        new_batch = DataProto.from_dict(batch_dict)
        
        # 复制并扩展non_tensor_batch（interleave方式）
        new_batch.non_tensor_batch = {}
        for key, value in batch.non_tensor_batch.items():
            # 每个元素重复3次（interleaved）
            expanded = np.array([
                value[i] for i in range(batch_size) for _ in range(n_agent)
            ], dtype=object)
            new_batch.non_tensor_batch[key] = expanded
        
        # 复制meta_info
        if hasattr(batch, 'meta_info'):
            new_batch.meta_info = batch.meta_info.copy()
        
        print(f"[INFO] Created multilingual GRPO batch: {batch_size} -> {len(all_input_ids)}")
        
        return new_batch
    # === 新增：按 GRPO 轮次切换 question 文本 ===
    def _rewrite_grpo_question_texts(self, batch: DataProto):
        """
        对已经过 repeat(interleave=True) 的 batch：
        同一条样本（通过 non_tensor_batch['index'] 识别）按出现顺序：
          第 1 次：content
          第 2 次：content_en
          第 3 次：content_source
          之后：回退到 content
        只改 non_tensor_batch['content'] 字段（给下游用，同步 meta 信息）。
        """
        # 这三列需要在 RLHFDataset 里从 parquet 读进来
        contents = batch.non_tensor_batch.get('content', None)
        contents_en = batch.non_tensor_batch.get('content_en', None)
        contents_src = batch.non_tensor_batch.get('content_source', None)

        if contents is None or contents_en is None or contents_src is None:
            # 数据集中缺字段就直接跳过，不报错
            return

        indices = batch.non_tensor_batch.get(
            'index',
            np.arange(len(contents))
        )

        counter = {}  # uid -> seen_times
        new_contents = []

        for uid, c, c_en, c_src in zip(indices, contents, contents_en, contents_src):
            seen = counter.get(uid, 0)

            if seen == 0:
                new_contents.append(c)       # 第 1 次：原语言
            elif seen == 1:
                new_contents.append(c_en)    # 第 2 次：英文版本
            elif seen == 2:
                new_contents.append(c_src)   # 第 3 次：高资源语言版本
            else:
                new_contents.append(c)       # 之后：退回原语言

            counter[uid] = seen + 1

        batch.non_tensor_batch['content'] = np.array(new_contents, dtype=object)
    ...
    def _create_dataloader(self):
        ...
        from verl.utils.dataset.rl_dataset_no import RLHFDataset, collate_fn
    ...
    def _validate(self):
        ...
        gen_config = GenerationConfig(
            ...
            num_gpus=self.config.trainer.n_gpus_per_node * self.config.trainer.nnodes,
            no_think_rl=self.config.algorithm.no_think_rl,
            search_url_base=self.config.retriever.search_url_base,
            topk = self.config.retriever.topk,
        )
        ...
        try:
                    ...
                    with torch.no_grad():  # 确保验证过程不计算梯度
                        ...
                        with _timer('step', timing_raw):
                            ...
                            with _timer('gen', timing_raw):
                                generation_manager.timing_raw = timing_raw
                                question_languages = []
                                for item in test_batch.non_tensor_batch.get('language', ['en'] * len(test_batch)):
            ...
            for metric_name in ['em', 'fem', 'c3recall']:
                ...
                for val, lg in zip(metric_values, languages_all):
                    lang_reward[lg].append(val)

                for lg, rewards in lang_reward.items():
                    avg = float(np.mean(rewards))
                    if metric_name == 'em':
                        final_metric_dict[f'val/lang/{lg}/em'] = avg
                        final_metric_dict[f'val/lang/{lg}/count'] = len(rewards)
                    elif metric_name == 'fem':
                        final_metric_dict[f'val/lang/{lg}/fem'] = avg
                    elif metric_name == 'c3recall':
                        final_metric_dict[f'val/lang/{lg}/c3recall'] = avg
            print(f"Final metrics to be logged: {final_metric_dict}")
    ...
    def _verify_multilingual_batch(self, batch: DataProto, step: int):
        """
        验证多语言batch是否正确创建
        在fit方法的开始调用一次即可
        """
        ...
        for i in range(min(6, batch_size)):
            decoded = self.tokenizer.decode(
                batch.batch['input_ids'][i],
                skip_special_tokens=False
            )
            
            sample_idx = i // n_agent
            version_idx = i % n_agent
            version_name = ['原语言', '英语', '高资源'][version_idx]
            
            print(f"\n样本{sample_idx} - 版本{version_idx}({version_name}):")
            print(f"  输入前100字符: {decoded[:100]}")
            print(f"  Language标签: {batch.non_tensor_batch.get('language', [None])[i]}")
            print(f"  Index: {batch.non_tensor_batch.get('index', [None])[i]}")
        
        print("="*60 + "\n")
        print("=== Original Batch Keys ===")
        print(f"non_tensor_batch keys: {batch.non_tensor_batch.keys()}")
        if 'content' in batch.non_tensor_batch:
            print(f"content[0]: {batch.non_tensor_batch['content'][0][:100]}")
        if 'content_en' in batch.non_tensor_batch:
            print(f"content_en[0]: {batch.non_tensor_batch['content_en'][0][:100]}")
        print("\n=== DEBUGGING FIRST BATCH ===")
        print(f"Batch keys: {batch.non_tensor_batch.keys()}")
        
        # 检查是否成功提取了三个版本
        for field in ['content', 'content_en', 'content_source']:
            if field in batch.non_tensor_batch:
                print(f"\n{field} exists: ✅")
                print(f"Sample: {batch.non_tensor_batch[field][0][:150]}...")
            else:
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                if self.config.algorithm.adv_estimator == 'grpo':
                    # GRPO模式：准备3个不同语言版本（已包含repeat效果）
                    batch = self._prepare_multilingual_grpo_batch(batch)
                    if self.global_steps == 1:
                        self._verify_multilingual_batch(batch, self.global_steps)
                else:
                    # 其他算法（PPO等）：标准repeat
                    batch = batch.repeat(
