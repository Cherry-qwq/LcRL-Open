...
class RayPPOTrainer(object):
    ...
    #     data_sources = np.concatenate(data_source_lst, axis=0)
    #     reward_tensor = reward_tensor.numpy()
    #     metric_dict = {}
    #     for data_source in np.unique(data_sources):
    #         mask = data_sources == data_source
    #         metric_dict[f'val/test_score/{data_source}'] = float(np.mean(reward_tensor[mask]))
        
    #     # === 新增：计算fem和c3recall（完全独立，不影响训练）===
    #     from verl.utils.reward_score import qa_em
        
    #     fem_scores = []
    #     c3recall_scores = []
        
    #     print(f"Computing additional metrics for {len(all_sequences)} samples...")
    #     for seq_str, gt in zip(all_sequences, all_ground_truths):
    #         fem_scores.append(qa_em.compute_score_fem(seq_str, gt))
    #         c3recall_scores.append(qa_em.compute_score_c3recall(seq_str, gt))
        
    #     fem_scores = np.array(fem_scores)
    #     c3recall_scores = np.array(c3recall_scores)
        
    #     # 按data_source分组添加额外指标
    #     for data_source in np.unique(data_sources):
    #         mask = data_sources == data_source
    #         metric_dict[f'val/test_score_fem/{data_source}'] = float(np.mean(fem_scores[mask]))
    #         metric_dict[f'val/test_score_c3recall/{data_source}'] = float(np.mean(c3recall_scores[mask]))
        
    #     print(f"Validation metrics: EM={metric_dict.get('val/test_score/mkqa', 0):.4f}, "
    #         f"FEM={metric_dict.get('val/test_score_fem/mkqa', 0):.4f}, "
    #         f"C3Recall={metric_dict.get('val/test_score_c3recall/mkqa', 0):.4f}")
