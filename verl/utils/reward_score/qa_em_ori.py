...
def compute_score_fem(solution_str, ground_truth):
    """Flexible Exact Match - 答案包含ground truth即可"""
    answer = extract_solution(solution_str=solution_str)
    if answer is None:
        return 0.0
    
    target = ground_truth.get('target', ground_truth) if isinstance(ground_truth, dict) else ground_truth
    ground_truths = target if isinstance(target, list) else [target]
    
    normalized_prediction = normalize_answer(answer)
    
    for gt in ground_truths:
        gt_str = str(gt) if not isinstance(gt, str) else gt
        normalized_gt = normalize_answer(gt_str)
        
        # 只要ground truth在预测中出现即可（标准化后比较）
...
def compute_score_c3recall(solution_str, ground_truth):
    ...
    for gt in ground_truths:
        ...
        if len(true_3grams) > 0:
            recall = len(pred_3grams & true_3grams) / len(true_3grams)
